...
public class PoemAnnotatorAltHtml {
	...
	ArrayList<AnnotatedPoemHtml> poemList = new ArrayList<AnnotatedPoemHtml>(); 
	HashMap<String,String> codeMap = new HashMap<String,String>();
	public static int batches  = 10;


	public static void main(String[] args) {
		PoemAnnotatorAltHtml obj = new PoemAnnotatorAltHtml();

		obj.codeMap.put("identical", "1");
		obj.codeMap.put("rich", "2");
		obj.codeMap.put("full", "3");
		obj.codeMap.put("slant", "4");		
		obj.codeMap.put("eye", "5");		
	...
	public void checkForEyeRhymes(){
		for(int i=0;i<poemList.size();i++){
			AnnotatedPoemHtml poem = poemList.get(i);
	...
	public double consectiveLineEyeRhyme(ArrayList<String> lastWordList, AnnotatedPoemHtml poem){
	...
	public double alternateLineEyeRhyme(ArrayList<String> lastWordList, AnnotatedPoemHtml poem){
		double totalRhymeScore = 0;
	...
	public double alternateLineIdenticalRhyme(ArrayList<String> lastWordList, AnnotatedPoemHtml poem){
		double totalRhymeScore = 0;
		//Check for alternate rhyming
		for(int k=2;k<lastWordList.size();k++){
			String currentLineEndWord = lastWordList.get(k);
	...
	public double consectiveLineIdenticalRhyme(ArrayList<String> lastWordList, AnnotatedPoemHtml poem){
		double totalRhymeScore = 0;
	...
	public double consectiveLineEndRhyme(ArrayList<String> lastWordList, AnnotatedPoemHtml poem){
		double totalRhymeScore = 0;
		//Check for consecutive rhyming
		for(int k=1;k<lastWordList.size();k++){
	...
	public double alternateLineEndRhyme(ArrayList<String> lastWordList, AnnotatedPoemHtml poem){
		double totalRhymeScore = 0;
	...
	public double internalRhyme(ArrayList<String> pauseWordList, ArrayList<String> lastWordList, ArrayList<String> pausePhonemeList, ArrayList<String> lastPhonemeList, AnnotatedPoemHtml poem){
		double totalRhymeScore = 0;
	...
	public void updateRhymeType(String rhymeType, AnnotatedPoemHtml poem, double rhymeScore, double normfactor){
	...
	public void readInputFolderPoetryFoundation(){
		...
		for(int loop=0; loop<children.length; loop++){
			poemList = new ArrayList<AnnotatedPoemHtml>();
			count++;
			...
			if(currentFile.isFile() && currentFile.getName().contains(".txt")){
				...
				AnnotatedPoemHtml poem = new AnnotatedPoemHtml();
				...
				ArrayList<String> origContent = new ArrayList<String>(); 
				...
				try{
					...
					while ((sCurrentLine = br.readLine()) != null) {
						...
						//Add original content for HTML rendering
						origContent.add(sOrigLine);
					}
					...
					poem.setOrigContent(origContent);
					br.close();
					br = null;
					currentFile = null;
				} catch (IOException e) {
			...
			addMarkTagToPoems();
			writeAnnotatedPoems("annotated_output_html/");
		...
		try {
			for(int i=0;i<poemList.size();i++){
				...
				for(int j=0;j<poem.getContent().size();j++){
					...
					//Changed to include Original content for HTML rendering
					//bw.write(poem.getOrigContent().get(j));

					//bw.write("\t\t");

					//Add mark tag to original poem content
					for(int s=0;s<currentAnno.size();s++){
						String currAnnoStr = currentAnno.get(s);
						if(!currAnnoStr.trim().equals("")){
							String []annoArr = currAnnoStr.split("=");
							try{
								if(annoArr[0].contains("cons")){
									String markCode = codeMap.get(annoArr[1].replaceAll("#", ""));
									String currentLine = poem.getOrigContent().get(j);
									String lastLine = poem.getOrigContent().get(j-1);
									addMarkTagToLine(lastLine.trim(), currentLine.trim(), j-1, j, poem, markCode);							
								}
								if(annoArr[0].contains("int")){
									String markCode = codeMap.get(annoArr[1].replaceAll("#", ""));
									String currentLine = poem.getOrigContent().get(j).trim();
									String []strArrTmp = currentLine.split(" ");
									int linePause = poem.getLinePause().get(j);
									//System.out.println(linePause);
									String anchorEl = strArrTmp[linePause];
									//System.out.println(anchorEl);
									int indexOfAnchor = currentLine.indexOf(anchorEl) + anchorEl.length();
									//System.out.println(indexOfAnchor);
									String firstPartLine = currentLine.substring(0, indexOfAnchor-1);
									String secondPartLine = currentLine.substring(indexOfAnchor+1, currentLine.length());
									addInternalMarkTagToLine(firstPartLine.trim(), secondPartLine.trim(), j, poem, markCode);
								}
								if(annoArr[0].contains("alt")){
									String markCode = codeMap.get(annoArr[1].replaceAll("#", ""));
									String currentLine = poem.getOrigContent().get(j);
									int altDistance = 1;
									if(!annoArr[0].equals("#alt")){
										altDistance = Integer.parseInt(annoArr[0].substring(annoArr[0].indexOf("alt")+3, annoArr[0].length()));
									}
									String lastLine = poem.getOrigContent().get(j-altDistance-1);
									addMarkTagToLine(lastLine.trim(), currentLine.trim(), j-altDistance-1, j, poem, markCode);
								}
							}catch(Exception ee){
								System.out.println("File:" + poem.getFileName());
								System.out.println("Line:" + poem.getOrigContent().get(j));
								ee.printStackTrace();
								System.exit(1);
							}
	...
	public void writeAnnotatedPoems(String str){
		...
		try {
			for(int i=0;i<poemList.size();i++){
				...
				//bw.write("\nFile Name : " + poem.getFileName());
				bw.write("\nTitle : " + poem.getTitle() + "<br>");
				bw.write("\nAuthor : " + poem.getAuthor() + "<br><br>\n\n");
				//bw.write(poem.printResult() + "\n");
				int incrementor = 1;
				for(int j=0;j<poem.getContent().size();j++){
	...
	public void addMarkTagToLine(String lastLine, String currentLine, int lastIndex, int currentindex, AnnotatedPoemHtml poem, String markCode) throws Exception{
		if(!lastLine.contains("<mark")){
			String cutString = lastLine.substring(0, lastLine.lastIndexOf(" "));
			String lastWord = lastLine.substring(lastLine.lastIndexOf(" "), lastLine.length());
			String finalStr = cutString + " <mark" + markCode + ">" + lastWord.trim() + "</mark" + markCode + ">";
			poem.setOrigContent(finalStr, lastIndex);
		}
		if(!currentLine.contains("<mark")){
			String cutString = currentLine.substring(0, currentLine.lastIndexOf(" "));
			String lastWord = currentLine.substring(currentLine.lastIndexOf(" "), currentLine.length());
			String finalStr = cutString + " <mark" + markCode + ">" + lastWord.trim() + "</mark" + markCode + ">";
			poem.setOrigContent(finalStr, currentindex);
		}
	}

	public void addInternalMarkTagToLine(String lastLine, String currentLine, int currentindex, AnnotatedPoemHtml poem, String markCode) throws Exception{
		String finalStr1 = lastLine;
		String finalStr2 = currentLine;
		if(!lastLine.contains("<mark")){
			if(lastLine.lastIndexOf(" ") != -1){
				String cutString = lastLine.substring(0, lastLine.lastIndexOf(" "));
				String lastWord = lastLine.substring(lastLine.lastIndexOf(" "), lastLine.length());
				finalStr1 = cutString + " <mark" + markCode + ">" + lastWord.trim() + "</mark" + markCode + ">";
			}else{
				finalStr1 = "<mark" + markCode + ">" + lastLine.trim() + "</mark" + markCode + ">";
			}
			//poem.setOrigContent(finalStr, lastIndex);
		}
		if(!currentLine.contains("<mark")){
			...
			if(currentLine.lastIndexOf(" ") != -1){
				String cutString = currentLine.substring(0, currentLine.lastIndexOf(" "));			
				String lastWord = currentLine.substring(currentLine.lastIndexOf(" "), currentLine.length());
				finalStr2 = cutString + " <mark" + markCode + ">" + lastWord.trim() + "</mark" + markCode + ">";
			}else{
				finalStr2 = "<mark" + markCode + ">" + currentLine.trim() + "</mark" + markCode + ">";
			}
			//poem.setOrigContent(finalStr, currentindex);
		}
		poem.setOrigContent(finalStr1 + ", " + finalStr2, currentindex);
	}

...
public class DictionQuantifier {

	public static String pofoPath = "D:/EclipseWorkSpace/GraphPoem/resources/poetry_foundation_final/";
	...
	public static void main(String[] args) {
		DictionQuantifier obj = new DictionQuantifier();
	...
	public void readInputFolderPoetryFoundation(){
		initializePOSModel();
		//String newline = System.getProperty("line.separator");
		String inputFolder = pofoPath;
		File inpFolder = new File(inputFolder);
		File[] children = inpFolder.listFiles();

		File outFile = new File(basePath + "diction_quantified.txt");
		FileWriter fw = null;
		...
		for(int loop=0; loop<children.length; loop++){
			...
			posCountMap = new HashMap<String, Integer>(); // for POS Counter
			wordList = new ArrayList<Word>(); // for Inflection Counter
			nounList = new HashSet<String>(); // for Abstract/Concrete Counter
			...
			if(currentFile.isFile() && currentFile.getName().contains(".txt")){
				...
				String sCurrentLine;
				ArrayList<String> poemContent = new ArrayList<String>();
				try{
					...
					while ((sCurrentLine = br.readLine()) != null) {
						...
						poemContent.add(sCurrentLine.trim());
					}

					String outStr = processLineArray(poemContent);
					
					bw.write(fileName + "," + outStr + "\n");
					bw.flush();
	...
	public String processLineArray(ArrayList<String> lineArr){

		for(int t=0;t<lineArr.size();t++){
			String line = lineArr.get(t);
	...
	public StringBuffer finalisePOSCounter(){
		...
		if(modPosCountMap.containsKey("VERB")){
			sb.append(formatNumbers((1.0*modPosCountMap.get("VERB")/modPosCountMap.get("TOKENS"))) + ",");
		}else{
			sb.append("0.000,");
		}
		if(modPosCountMap.containsKey("NOUN")){
			sb.append(formatNumbers((1.0*modPosCountMap.get("NOUN")/modPosCountMap.get("TOKENS"))) + ",");
		}else{
			sb.append("0.000,");
		}
		if(modPosCountMap.containsKey("ADJ")){
			sb.append(formatNumbers((1.0*modPosCountMap.get("ADJ")/modPosCountMap.get("TOKENS"))) + ",");
		}else{
			sb.append("0.000,");
		}
		if(modPosCountMap.containsKey("PRON") && modPosCountMap.containsKey("NOUN")){
			sb.append(formatNumbers((1.0*modPosCountMap.get("PRON")/modPosCountMap.get("NOUN"))) + ",");
		}else{
			sb.append("0.000,");
		}
		return sb;
	...
	public StringBuffer finaliseInflection(){
		...
		sb.append(formatNumbers(inflectionList.size()*1.0/noninflectionList.size()) + ",");
		return sb;
	}
	...
	public StringBuffer finaliseAbstractConcrete(){
		...
		sb.append(formatNumbers(abstractList.size()*1.0/concreteList.size()));
		return sb;
	}
